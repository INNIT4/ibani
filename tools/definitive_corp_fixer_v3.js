...
plans.forEach(plan => {
    ...
    // 2. Hero
    html = html.replace(/0?([1-6]) de 6 · Corporativos/g, `${plan.step} de 6 · Corporativos`);
    ...
    // 3. Includes list
    html = html.replace(/\d+ páginas maestras/g, `${plan.pages} páginas maestras`);
    html = html.replace(/Entrega en .*? días hábiles/g, `Entrega en 15 días hábiles`);
    ...
    // 6. Contact Section (The one at the bottom with dark background)
    html = html.replace(/<span class="label" style="color:rgba\(255,255,255,\.3\)">Sitio .*? · \$.*? · Entrega en .*?<\/span>/, 
        `<span class="label" style="color:rgba(255,255,255,.3)">Sitio ${plan.name} · $${plan.price} MXN · Entrega en 15 días</span>`);
    
    html = html.replace(/Comprar Sitio .*? <\/a>/, `Comprar Sitio ${plan.name} </a>`);

    // 7. Navigation (Next/Prev plans at the bottom)
    // No change needed if links are correct, but let's ensure prices match the names in nav
    html = html.replace(/<span class="plan-nav__item-name">Sitio Bronce<\/span>\s*<span class="plan-nav__item-price">.*?<\/span>/g, '<span class="plan-nav__item-name">Sitio Bronce</span>\n            <span class="plan-nav__item-price">$22,000 MXN</span>');
    html = html.replace(/<span class="plan-nav__item-name">Sitio Silver<\/span>\s*<span class="plan-nav__item-price">.*?<\/span>/g, '<span class="plan-nav__item-name">Sitio Silver</span>\n            <span class="plan-nav__item-price">$25,000 MXN</span>');
    html = html.replace(/<span class="plan-nav__item-name">Sitio Gold<\/span>\s*<span class="plan-nav__item-price">.*?<\/span>/g, '<span class="plan-nav__item-name">Sitio Gold</span>\n            <span class="plan-nav__item-price">$29,000 MXN</span>');
    html = html.replace(/<span class="plan-nav__item-name">Sitio Platino<\/span>\s*<span class="plan-nav__item-price">.*?<\/span>/g, '<span class="plan-nav__item-name">Sitio Platino</span>\n            <span class="plan-nav__item-price">$34,000 MXN</span>');
    html = html.replace(/<span class="plan-nav__item-name">Sitio Diamante<\/span>\s*<span class="plan-nav__item-price">.*?<\/span>/g, '<span class="plan-nav__item-name">Sitio Diamante</span>\n            <span class="plan-nav__item-price">$38,000 MXN</span>');
    html = html.replace(/<span class="plan-nav__item-name">Sitio Esmeralda<\/span>\s*<span class="plan-nav__item-price">.*?<\/span>/g, '<span class="plan-nav__item-name">Sitio Esmeralda</span>\n            <span class="plan-nav__item-price">$44,500 MXN</span>');
    ...
    html = html.replace(/wa\.me\/526625044016\?text=.*?["\s]/g, (match) => {
        ...
        return `wa.me/526625044016?text=${waText}${quote}`;
    });

    fs.writeFileSync(filePath, html);
    console.log(`Plan ${plan.name} fixed (v3) successfully.`);
});
